...
async function createImagesSection(postData: any): Promise<Paragraph[]> {
  ...
  for (let i = 0; i < postData.length; i++) {
    ...
    if (item.fotoPrincipal) {
      try {
        console.log(
          `Processing image ${i}: ${item.fotoPrincipal?.substring(0, 30)}...`
        );
        ...
        // Add the image with proper data format
        paragraphs.push(
          new Paragraph({
            children: [
              new ImageRun({
                data: new Uint8Array(imageData), // Convert ArrayBuffer to Uint8Array
                transformation: {
                  width: 500,
                  height: 350,
                },
        ...
        paragraphs.push(
          new Paragraph({
            children: [
              new TextRun(`[Error al cargar imagen ${i + 1}: ${error}]`),
            ],
            spacing: { after: 100 },
          })
        );
